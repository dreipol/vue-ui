...
describe('Vuex overlay module', () => {
  ...
  describe('Actions', () => {
    describe('openOverlay', () => {
      it('Opening a new overlay dispatches all mutations properly', async () => {
        const commit = spy()
        const state = { overlays: {} }

        await actions.openOverlay(
          { commit, state },
          {
            id: 'foo',
        ...
        const [
          openOverlayEventMutationType,
          openOverlayEventData,
        ] = openOverlayEventArgs

        expect(mountOverlayEventArgs).to.be.deep.equal([
          MOUNT_OVERLAY,
          { id: 'foo' },
        ])
        ...
        expect(openOverlayEventData.props.title).to.be.equal(
          'bar',
          'The payload was properly forwarded',
        )
      })

      it('Opening the same overlay twice will not dispatch the mounting mutation again', async () => {
        const commit = spy()
        const state = {
          overlays: {
            foo: { props: { title: 'bar' } },
    ...
    describe('updateOverlay', () => {
      it('Updating an existing overlay dispatches all mutations properly', async () => {
        const commit = spy()
        const state = {
          overlays: {
            foo: { props: { title: 'bar' } },
        ...
        const [
          updateOverlayEventMutationType,
          updateOverlayEventData,
        ] = commit.args[0]
        ...
        expect(updateOverlayEventData.props.title).to.be.equal(
          'bar',
          'The payload was properly forwarded',
        )
      })
    ...
    describe('closeOverlay', () => {
      it('Closing an overlay properly dispatches all mutations', async () => {
        const commit = spy()
        const state = {
          overlays: {
            foo: { props: { title: 'bar' } },
      ...
      it('Closing an overlay allows to asynchronously dispatch the unmount event', async () => {
        const commit = spy()
        const state = {
          overlays: {
        ...
        const [
          unmountOverlayEventMutationType,
          umountOverlayEventData,
        ] = unmountOverlayEvent
      ...
      it('Closing an overlay allows for custom transitions', async () => {
        const commit = spy()
        const state = {
          overlays: {
            foo: { id: 'foo', props: { title: 'bar' } },
        ...
        await actions.closeOverlay(
          { commit, state },
          { id: 'foo', transition: 'my-cool-transition' },
        )
      ...
      it('Closing unknown overlays will not dispatch any event', async () => {
        const commit = spy()
        const state = { overlays: {} }
  ...
  describe('Mutations', () => {
    ...
    it('Open overlay', () => {
      ...
      overlayModule.mutations[OPEN_OVERLAY](state, {
        id: 'foo',
        props: { title: 'bar' },
      })
